...
export class DatabaseServiceImpl {
    ...
    async getAssistantLogs(limit: number = 50): Promise<AssistantLog[]> {
        return await this.database.assistantLogs
            .orderBy('timestamp')
            .reverse()
            .limit(limit)
            .toArray();
    ...
    async importData(
        jsonData: string
    ): Promise<{ success: boolean; errors: string[]; stats: ImportValidationResult['stats'] }> {
        ...
        await this.database.transaction(
            ...
            async () => {
                ...
                if (Array.isArray(data.dailyInputs) && data.dailyInputs.length <= 1000) {
                    await this.database.dailyInputs.bulkPut(
                        data.dailyInputs as (DailyInput & { id: string })[]
                    );
                }
                if (Array.isArray(data.plans) && data.plans.length <= 1000) {
                    await this.database.plans.bulkPut(
                        data.plans as (PlanOutput & { id: string })[]
                    );
                }
                ...
                if (
                    Array.isArray(data.tomorrowSuggestions) &&
                    data.tomorrowSuggestions.length <= 100
                ) {
                    await this.database.tomorrowSuggestions.bulkPut(
                        data.tomorrowSuggestions as TomorrowSuggestion[]
                    );
                }
...
export const DatabaseServiceStatic = {
    ...
    saveTomorrowSuggestion: (suggestion: TomorrowSuggestion) =>
        databaseService.saveTomorrowSuggestion(suggestion),
    ...
    getDailyInput: (date: string, timezone: string) =>
        databaseService.getDailyInput(date, timezone),

    // Plans operations
    savePlan: (plan: PlanOutput) => databaseService.savePlan(plan),
    getPlan: (date: string) => databaseService.getPlan(date),
    getPlansInRange: (startDate: string, endDate: string) =>
    ...
    getHistoryInRange: (startDate: string, endDate: string) =>
