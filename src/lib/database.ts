...
export class DatabaseServiceImpl {
  ...
  /**
   * Compute and persist quest XP for a given day based on the saved plan.
   * This is local-only and does not affect scheduling behavior.
   */
  async upsertQuestStatsFromPlan(date: string, plan: PlanOutput): Promise<QuestStats> {
    const completed = (plan.blocks ?? []).filter(
      b => b.completed && (b.type === 'task' || b.type === 'habit' || b.type === 'gym')
    );

    const taskIds = Array.from(
      new Set(completed.filter(b => b.type === 'task' && b.sourceId).map(b => b.sourceId!))
    );
    const habitIds = Array.from(
      new Set(completed.filter(b => b.type === 'habit' && b.sourceId).map(b => b.sourceId!))
    );

    const tasks = await Promise.all(taskIds.map(id => this.database.tasks.get(id)));
    const habits = await Promise.all(habitIds.map(id => this.database.habits.get(id)));

    const taskPriority = new Map(tasks.filter(Boolean).map(t => [t!.id, t!.priority]));
    const habitPriority = new Map(habits.filter(Boolean).map(h => [h!.id, h!.priority]));
    ...
    for (const b of completed) {
      ...
      const pri =
        b.type === 'task'
          ? (b.sourceId ? (taskPriority.get(b.sourceId) ?? 3) : 3)
          : b.type === 'habit'
            ? (b.sourceId ? (habitPriority.get(b.sourceId) ?? 3) : 3)
            : 4;
      xp += computeXP(dur, pri);
    }

    const completedCount = completed.length;

    const yesterday = format(subDays(parseISO(date), 1), 'yyyy-MM-dd');
    const prev = await this.database.questStats.get(yesterday);
    const streak =
      completedCount > 0
        ? ((prev && prev.completedCount > 0) ? prev.streak + 1 : 1)
        : 0;
  ...
  async importData(jsonData: string): Promise<{ success: boolean; errors: string[]; stats: ImportValidationResult['stats'] }> {
    ...
    await this.database.transaction('rw', [this.database.habits, this.database.tasks, this.database.dailyInputs, this.database.plans, this.database.preferences, this.database.history, this.database.assistantLogs, this.database.tomorrowSuggestions, this.database.featureFlags, this.database.questStats], async () => {
