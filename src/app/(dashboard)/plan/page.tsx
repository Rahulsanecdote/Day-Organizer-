                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                onChange={e =>
                                    setEditedBlock({ ...editedBlock, start: e.target.value })
                                }
                                className="w-full px-4 py-3 rounded-lg text-sm focus:outline-none"
                                style={{
                                    background: 'var(--color-surface)',
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={e =>
                                    setEditedBlock({ ...editedBlock, end: e.target.value })
                                }
                                className="w-full px-4 py-3 rounded-lg text-sm focus:outline-none"
                                style={{
                                    background: 'var(--color-surface)',
                        ...
                        <div className="flex gap-2">
                            {energyLevels.map(level => (
                                <button
                                    key={level}
                                    onClick={() =>
                                        setEditedBlock({ ...editedBlock, energyLevel: level })
                                    }
                                    className="flex-1 py-2 rounded-lg text-sm font-medium transition-all"
                                    style={{
                                        background:
                                            editedBlock.energyLevel === level
                                                ? 'var(--color-ivory)'
                                                : 'transparent',
            ...
            >
                ...
                <div className="space-y-5">
                    ...
                    <div>
                        ...
                        <select
                            value={newBlock.type}
                            onChange={e =>
                                setNewBlock({ ...newBlock, type: e.target.value as BlockType })
                            }
                            className="w-full px-4 py-3 rounded-lg text-sm focus:outline-none"
                            style={{
                                background: 'var(--color-surface)',
                ...
                <div className="flex gap-3 mt-8">
                    ...
                    <button
                        ...
                        style={{
                            background: newBlock.title
                                ? 'linear-gradient(135deg, var(--color-gold) 0%, var(--color-gold-dark) 100%)'
                                : 'var(--color-ivory)',
                            color: newBlock.title ? 'white' : 'var(--color-mist)',
                            boxShadow: newBlock.title
...
export default function PlanPage() {
    ...
    const handleSmartReoptimize = async () => {
        ...
        try {
            ...
            if (!dailyInput) {
                showToast('No daily input found. Please set up your day first.', 'error');
                return;
            }
            ...
            const lockedAsFixed = lockedBlocks
            ...
            const mergedEvents = [
                ...
                ...googleEvents
                    .filter(e => !existingIds.has(`${e.start}-${e.end}`))
                    .map(e => ({
                        ...e,
            ...
            const preservedBlocks = [
                ...lockedBlocks,
                ...newPlan.blocks.filter(
                    b => !lockedBlocks.some(lb => lb.start === b.start && lb.end === b.end)
                ),
            ...
            showToast('Failed to reoptimize plan. Please try again.', 'error');
        } finally {
    ...
    const getBlockStyle = (type: ScheduledBlock['type']) => {
        const styles: Record<string, { bg: string; border: string; accent: string }> = {
            work: {
                bg: 'rgba(122, 158, 159, 0.08)',
                border: 'var(--color-work)',
                accent: 'var(--color-work)',
            },
            gym: {
                bg: 'rgba(139, 159, 130, 0.08)',
                border: 'var(--color-gym)',
                accent: 'var(--color-gym)',
            },
            habit: {
                bg: 'rgba(155, 138, 160, 0.08)',
                border: 'var(--color-habit)',
                accent: 'var(--color-habit)',
            },
            task: {
            ...
            meal: {
                bg: 'rgba(201, 150, 126, 0.08)',
                border: 'var(--color-meal)',
                accent: 'var(--color-meal)',
            },
            break: {
                bg: 'rgba(163, 158, 154, 0.08)',
                border: 'var(--color-break)',
                accent: 'var(--color-break)',
            },
            sleep: {
                bg: 'rgba(136, 146, 168, 0.08)',
                border: 'var(--color-sleep)',
                ...
                return (
                    ...
                    >
                        <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            d="M12 8.25v-1.5m0 1.5c-1.355 0-2.697.056-4.024.166C6.845 8.51 6 9.473 6 10.608v2.513m6-4.871c1.355 0 2.697.056 4.024.166C17.155 8.51 18 9.473 18 10.608v2.513M15 8.25v-1.5m-6 1.5v-.75A2.25 2.25 0 0 0 6.75 6H6A2.25 2.25 0 0 1 3.75 3.75v-.75"
                        />
                    </svg>
                );
            case 'break':
            ...
            <div className="max-w-4xl mx-auto">
                ...
                >
                    <p style={{ color: 'var(--color-slate)' }}>
                        No plan found for today. Please set up your day first.
                    </p>
                    <button
                ...
                >
                    ...
                    {isRegularPlan(plan.stats) ? (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
                            {[
                                {
                                    value: `${plan.stats.workHours}h`,
                                    label: 'Work',
                                    color: 'var(--color-work)',
                                },
                                {
                                    value: `${plan.stats.gymMinutes}m`,
                                    label: 'Exercise',
                                    color: 'var(--color-gym)',
                                },
                                {
                                    value: plan.stats.habitsCompleted,
                                    label: 'Habits',
                                    color: 'var(--color-habit)',
                                },
                                {
                                    value: plan.stats.tasksCompleted,
                                    label: 'Tasks',
                                    color: 'var(--color-task)',
                                },
                                {
                                    value: plan.stats.focusBlocks,
                                    label: 'Focus Blocks',
                                    color: 'var(--color-gold)',
                                },
                                {
                                    value: `${plan.stats.freeTimeRemaining}m`,
                                    label: 'Free Time',
                        ...
                        <div className="grid grid-cols-3 gap-6">
                            {[
                                {
                                    value: `${plan.stats.totalFocusTimeMinutes}m`,
                                    label: 'Focus Time',
                                    color: 'var(--color-sleep)',
                                },
                                {
                                    value: `${plan.stats.totalFreeTimeMinutes}m`,
                                    label: 'Free Time',
                                    color: 'var(--color-mist)',
                                },
                                {
                                    value: `${Math.round(plan.stats.completionRate * 100)}%`,
                                    label: 'Completion',
                                    color: 'var(--color-gold)',
                                },
                            ].map((stat, index) => (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {(['time', 'type'] as const).map(option => (
                            <button
                                ...
                                style={{
                                    background:
                                        sortBy === option ? 'var(--color-ivory)' : 'transparent',
                                    color:
                                        sortBy === option
                                            ? 'var(--color-charcoal)'
                                            : 'var(--color-mist)',
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {(['all', 'incomplete'] as const).map(option => (
                            <button
                                ...
                                style={{
                                    background:
                                        filterBy === option ? 'var(--color-ivory)' : 'transparent',
                                    color:
                                        filterBy === option
                                            ? 'var(--color-charcoal)'
                                            : 'var(--color-mist)',
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3
                                                        ...
                                                        style={{
                                                            color: 'var(--color-charcoal)',
                                                            textDecoration: block.completed
                                                                ? 'line-through'
                                                                : 'none',
                                                        }}
                                                    >
                                            ...
                                            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <button
                                                    onClick={e => {
                                                        e.stopPropagation();
                                                        handleToggleCompletion(block.id);
                                                    }}
                                                    className="p-2 rounded-lg transition-colors"
                                                    style={{
                                                        background: block.completed
                                                            ? 'rgba(139, 159, 130, 0.15)'
                                                            : 'transparent',
                    ...
                    <ul className="space-y-3">
                        {plan.nextDaySuggestions.map((suggestion, index) => (
                            <li key={index} className="flex items-start gap-3">
                                <span className="mt-0.5" style={{ color: 'var(--color-gold)' }}>
                                    <svg
                                        className="w-4 h-4"
                                        fill="none"
                                        viewBox="0 0 24 24"
