...
export default function HistoryPage() {
    ...
    const calculateBestStreak = useCallback((historyData: DayHistory[]): number => {
        ...
        for (const day of sortedData) {
            const completionRate =
                day.completionStats.totalCompleted /
                Math.max(day.completionStats.totalScheduled, 1);
    ...
    const calculateWeeklyStats = useCallback(
        (historyData: DayHistory[], plansData: PlanOutput[], questStatsData: QuestStats[]) => {
            if (historyData.length === 0) return;

            const totalDays = historyData.length;
            const totalHabits = historyData.reduce(
                (sum, day) => sum + day.completionStats.habitsCompleted,
                0
            );
            const totalTasks = historyData.reduce(
                (sum, day) => sum + day.completionStats.tasksCompleted,
                0
            );
            ...
            const totalXP = questStatsData.reduce((sum, stats) => sum + (stats.xp ?? 0), 0);
            const bestXPStreak = questStatsData.reduce(
                (max, stats) => Math.max(max, stats.streak ?? 0),
                0
            );
    ...
    const loadHistory = useCallback(async () => {
        ...
        try {
            ...
            calculateWeeklyStats(historyData, plansData, questStatsData);
        } finally {
            setIsLoading(false);
        }
    ...
    const getCompletionRate = (day: DayHistory): number => {
        ...
        return Math.round(
            (day.completionStats.totalCompleted / day.completionStats.totalScheduled) * 100
        );
    };
        ...
        return (
            <div className="max-w-6xl mx-auto space-y-6">
                ...
                >
                    ...
                    />
                    <div
                        className="h-4 w-64 rounded mt-2"
                        style={{ background: 'var(--color-ivory)' }}
                    />
                </div>
                ...
                <div className="flex space-x-4">
                    {[
                        ...
                        <button
                            ...
                            style={{
                                background:
                                    selectedPeriod === key ? 'var(--color-ivory)' : 'transparent',
                                color:
                                    selectedPeriod === key
                                        ? 'var(--color-charcoal)'
                                        : 'var(--color-mist)',
                    ...
                    <div className="flex items-center">
                        <div className="flex-shrink-0">
                            <div
                                className="w-10 h-10 rounded-lg flex items-center justify-center"
                                style={{
                                    background: 'rgba(155, 138, 160, 0.1)',
                        ...
                        <div className="ml-4">
                            ...
                            <p
                                ...
                                style={{ color: 'var(--color-charcoal)' }}
                            >
                                {weeklyStats.avgHabitsCompleted}
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex items-center">
                        <div className="flex-shrink-0">
                            <div
                                className="w-10 h-10 rounded-lg flex items-center justify-center"
                                style={{
                                    background: 'rgba(139, 159, 130, 0.1)',
                                    color: 'var(--color-gym)',
                                }}
                            >
                                <span className="text-lg">💪</span>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                                <div>
                                                    ...
                                                    >
                                                        {day.completionStats.habitsCompleted}
                                                    </span>
                                                </div>
                                                <div>
                                                    <span style={{ color: 'var(--color-mist)' }}>
                                                    ...
                                                    <span
                                                        ...
                                                        style={{ color: 'var(--color-stone)' }}
                                                    >
                                                        {day.completionStats.tasksCompleted}
                                                    </span>
                                                </div>
                                                <div>
                                                    <span style={{ color: 'var(--color-mist)' }}>
                                                    ...
                                                    >
                                                        {day.completionStats.totalCompleted}/
                                                        {day.completionStats.totalScheduled}
                                                    </span>
                                                </div>
                                                <div>
                                                    <span style={{ color: 'var(--color-mist)' }}>
                                            ...
                                            {(day.mood || day.energy) && (
                                                <div
                                                    className="flex space-x-4 mt-3 text-sm"
                                                    style={{ color: 'var(--color-stone)' }}
                                                >
                                                    {day.mood && (
                ...
                >
                    ...
                    <div className="space-y-4">
                        {weeklyStats.currentStreak === 0 && (
                            ...
                            >
                                <div className="flex items-start">
                                    <span style={{ color: 'var(--color-work)' }} className="mr-3">
                                        💡
                                    </span>
                                    <div>
                                        <h4
                                        ...
                                        <p
                                            ...
                                            style={{ color: 'var(--color-slate)' }}
                                        >
                                            Complete 80% or more of your scheduled items today to
                                            start a new streak!
                                        </p>
                                    </div>
                                </div>
                        ...
                        {weeklyStats.avgHabitsCompleted < 3 && (
                            ...
                            >
                                <div className="flex items-start">
                                    <span style={{ color: 'var(--color-task)' }} className="mr-3">
                                        ⚠️
                                    </span>
                                    <div>
                                        <h4
                                        ...
                                        <p
                                            ...
                                            style={{ color: 'var(--color-slate)' }}
                                        >
                                            You&apos;re completing an average of{' '}
                                            {weeklyStats.avgHabitsCompleted} habits per day.
                                            Consider adjusting habit frequency or duration for
                                            better consistency.
                                        </p>
                                    </div>
                                </div>
                        ...
                        {weeklyStats.avgTasksCompleted < 2 && (
                            ...
                            >
                                <div className="flex items-start">
                                    <span style={{ color: 'var(--color-meal)' }} className="mr-3">
                                        🎯
                                    </span>
                                    <div>
                                        <h4
                                        ...
                                        >
                                            You&apos;re completing an average of{' '}
                                            {weeklyStats.avgTasksCompleted} tasks per day. Try
                                            breaking larger tasks into smaller, more manageable
                                            chunks.
                                        </p>
                                    </div>
                                </div>
                        ...
                        {weeklyStats.totalGymMinutes < 120 && (
                            ...
                            >
                                <div className="flex items-start">
                                    <span style={{ color: 'var(--color-gym)' }} className="mr-3">
                                        💪
                                    </span>
                                    <div>
                                        <h4
                                        ...
                                        <p
                                            ...
                                            style={{ color: 'var(--color-slate)' }}
                                        >
                                            You&apos;ve logged {weeklyStats.totalGymMinutes} minutes
                                            of gym time this period. Consider setting up regular gym
                                            sessions for better health tracking.
                                        </p>
                                    </div>
                                </div>
                            </div>
