...
export default function TodaySetupPage() {
    ...
    const [newEvent, setNewEvent] = useState<
        Omit<FixedEvent, 'title' | 'start' | 'end'> & { title: string; start: string; end: string }
    >({
        type: 'work',
        title: '',
        start: '',
    ...
    const loadUserData = useCallback(async () => {
        ...
        const existingInput = await DataService.getDailyInput(dailyInput.date, dailyInput.timezone);
        if (existingInput) {
    ...
    const handleParseText = () => {
        ...
        setTextInput('');
        showToast(
            `Parsed ${newEvents.length} events. ${
                parsed.unparsedText
                    ? `Could not parse: ${parsed.unparsedText}`
                    : 'All text parsed successfully!'
            }`,
            parsed.unparsedText ? 'error' : 'success'
        );
    };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input
                            ...
                            onChange={e =>
                                setDailyInput(prev => ({ ...prev, date: e.target.value }))
                            }
                    ...
                    <div>
                        ...
                        <input
                            ...
                            onChange={e =>
                                setDailyInput(prev => ({ ...prev, timezone: e.target.value }))
                            }
                            className="w-full px-4 py-3 rounded-lg text-sm focus:outline-none transition-all duration-200"
                            style={{
                                background: 'var(--color-surface)',
                    ...
                    <div className="flex gap-2">
                        {['form', 'text'].map(mode => (
                            <button
                                ...
                                style={{
                                    background:
                                        inputMode === mode ? 'var(--color-ivory)' : 'transparent',
                                    color:
                                        inputMode === mode
                                            ? 'var(--color-charcoal)'
                                            : 'var(--color-mist)',
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={e =>
                                        setNewEvent(prev => ({
                                            ...prev,
                                            type: e.target.value as FixedEvent['type'],
                                        }))
                                    }
                                    className="w-full px-4 py-3 rounded-lg text-sm focus:outline-none transition-all duration-200"
                                    style={{
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <p className="text-xs mt-2" style={{ color: 'var(--color-mist)' }}>
                                Format: &quot;Event HH:MM-HH:MM&quot; or &quot;Event
                                HH:MMam-HH:MMpm&quot;
                            </p>
                        </div>
                        <button
