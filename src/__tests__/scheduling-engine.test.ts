    ...
    describe('buffer constraints', () => {
        it('should maintain minimum buffer between consecutive blocks', () => {
            ...
            for (let i = 0; i < sortedBlocks.length - 1; i++) {
                ...
                const currentEndMinutes =
                    parseInt(current.end.split(':')[0]) * 60 + parseInt(current.end.split(':')[1]);
                const nextStartMinutes =
                    parseInt(next.start.split(':')[0]) * 60 + parseInt(next.start.split(':')[1]);
        ...
        it('should maintain buffer between fixed events and scheduled items', () => {
            ...
            const inputWithWork: DailyInput = {
                ...
                fixedEvents: [{ title: 'Work', start: '06:30', end: '13:00', type: 'work' }],
                constraints: {
                    buffersBetweenBlocksMin: 10,
                    protectDowntimeMin: 30,
                },
            ...
            if (gymBlock && workBlock) {
                const workEndMinutes =
                    parseInt(workBlock.end.split(':')[0]) * 60 +
                    parseInt(workBlock.end.split(':')[1]);
                const gymStartMinutes =
                    parseInt(gymBlock.start.split(':')[0]) * 60 +
                    parseInt(gymBlock.start.split(':')[1]);
    ...
    describe('real-time anchoring', () => {
        ...
        it('should anchor to current time + buffer when planning for today (mid-day)', () => {
            ...
            if (firstBlock) {
                // Should be >= 12:15
                const startMinutes =
                    parseInt(firstBlock.start.split(':')[0]) * 60 +
                    parseInt(firstBlock.start.split(':')[1]);
